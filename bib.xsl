...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="tabelle">
    <FMPXMLRESULT>
      <METADATA>
        <xsl:apply-templates select="tabella[@nomeFM='BIBLIOGRAFIE']"/>
      </METADATA>
      <RESULTSET>
        <xsl:apply-templates select="document('scheda_oa.xml')//BIB"/>
      </RESULTSET>
    </FMPXMLRESULT>
  </xsl:template>
  ...
  <xsl:template match="BIB">
    ...
    <ROW>
      ...
      <xsl:for-each select="document('mappa_campi.xml')//tabella[@nomeFM='BIBLIOGRAFIE']/campo">
