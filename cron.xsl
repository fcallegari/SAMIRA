...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="tabelle">
    <FMPXMLRESULT>
      <METADATA>
        <xsl:apply-templates select="tabella[@nomeFM='CRONOLOGIE']"/>
      </METADATA>
      <RESULTSET>
        <xsl:apply-templates select="document('scheda_oa.xml')//DT"/>
      </RESULTSET>
    </FMPXMLRESULT>
  </xsl:template>
  ...
  <xsl:template match="DT">
    ...
    <xsl:variable name="chiave" select="@id_section"/>

    <xsl:variable name="chiave_esterna" select="ancestor::CARD//NCI"/>
    <ROW>

      <xsl:for-each select="document('mappa_campi.xml')//tabella[@nomeFM='CRONOLOGIE']/campo">
