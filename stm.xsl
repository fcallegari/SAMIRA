...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="tabelle">
    <FMPXMLRESULT>
      <METADATA>
        <xsl:apply-templates select="tabella[@nomeFM='STEMMI_EMBLEMI']"/>
      </METADATA>
      <RESULTSET>
        <xsl:apply-templates select="document('scheda_oa.xml')//STM"/>
      </RESULTSET>
    </FMPXMLRESULT>
  </xsl:template>
  ...
  <xsl:template match="STM">
    ...
    <ROW>
      ...
      <xsl:for-each select="document('mappa_campi.xml')//tabella[@nomeFM='STEMMI_EMBLEMI']/campo">
